...
/deli:subtask FEATURE_KEY WT-001 planned
/deli:subtask FEATURE_KEY WT-001 in_progress
/deli:subtask FEATURE_KEY WT-001 done
/deli:ci FEATURE_KEY
```
...
Перед созданием бизнес-требования прочитай корневой `AGENTS.md` целевого проекта, если он есть, и учитывай его правила при формулировке требований, ограничений, критериев готовности и требований к тестам.
...
Для S2T/data mapping задач тесты создаются по Excel-файлу из папки `s2t`: прочитай лист `Target_columns`, при необходимости временно конвертируй его в CSV, создай итоговый CSV тестов в формате `Id:Entity:Test_name:Test_script:Expected_result`, не выводи чувствительные данные в чат и удали все временные файлы после работы.
...
Для S2T/data mapping обязательные SQL-проверки: кардинальность по всем таблицам, заполненность обязательных атрибутов с `col null = NO`, дубли по первичным ключам и битые ссылки по FK. Для каждого FK определи соответствующий PK; если связь неочевидна, задай пользователю конкретный вопрос и не угадывай.
...
Если runtime возвращает `subtask_id`, продолжай с указанной внутренней подзадачи в той же feature branch.
...
- Если фича большая или имеет `size: L`, разбей ее на work items и фиксируй состояние каждого work item в `.deli/state.json` через `/deli:subtask`.
...
- Перед созданием бизнес-требования прочитай корневой `AGENTS.md` целевого проекта, если файл существует, и учитывай его при подготовке документа.
...
python "%USERPROFILE%\.gigacode\extensions\delion\main.py" --project-root "%CD%" /deli:subtask BR-001 WT-001 planned --title "Описание подзадачи"
python "%USERPROFILE%\.gigacode\extensions\delion\main.py" --project-root "%CD%" /deli:subtask BR-001 WT-001 in_progress
python "%USERPROFILE%\.gigacode\extensions\delion\main.py" --project-root "%CD%" /deli:subtask BR-001 WT-001 done --note "Кратко что сделано"
...
Python runtime ограничен локальными операциями с документами и состоянием: `init`, `feature`, `validate`, `mark`, `subtask`, `resume`, `status`.
...
- checkpoint-и и состояние work items хранятся в `.deli/state.json`.
