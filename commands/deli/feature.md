...
```powershell
python "$HOME/.gigacode/extensions/delion/main.py" \deli:feature FEATURE_KEY @path/to/file.md
```

Если пользователь передал текст задачи, выполни:

```powershell
python "$HOME/.gigacode/extensions/delion/main.py" \deli:feature FEATURE_KEY "Текст задачи"
```
