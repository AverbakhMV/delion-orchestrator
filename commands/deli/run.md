...
3. Реализуй production-код одним основным execution agent.
   ...
   Если фича большая, рискованная или в бизнес-требовании указан `size: L`, сначала разбей реализацию на внутренние work items. Work items должны быть небольшими проверяемыми подзадачами внутри той же feature branch и того же PR.

   Для каждой подзадачи создай запись в state до начала реализации:

```powershell
python "%USERPROFILE%\.gigacode\extensions\delion\main.py" --project-root "%CD%" /deli:subtask FEATURE_KEY WT-001 planned --title "Краткое название подзадачи" --branch "BRANCH_NAME"
```

   Перед началом работы над подзадачей отметь ее как `in_progress`:

```powershell
python "%USERPROFILE%\.gigacode\extensions\delion\main.py" --project-root "%CD%" /deli:subtask FEATURE_KEY WT-001 in_progress --branch "BRANCH_NAME"
```

   После завершения подзадачи отметь ее как `done`:

```powershell
python "%USERPROFILE%\.gigacode\extensions\delion\main.py" --project-root "%CD%" /deli:subtask FEATURE_KEY WT-001 done --note "Кратко что сделано" --branch "BRANCH_NAME"
```

   Если подзадача заблокирована, отметь `blocked` и укажи причину через `--last-error`. При `/deli:resume` агент должен продолжить с первой подзадачи, которая не имеет статус `done`.
...
```
   ...
   Не отмечай общий checkpoint `implemented`, пока все обязательные work items этой фичи не имеют статус `done`.
...
- Не теряй состояние внутренних work items: для большой фичи каждая подзадача должна быть записана в `.deli/state.json` через `/deli:subtask`.
